Shipping = {
	...
	show_shipping: function(data){
		if(data.length > 0){
			...
			$.each(data,function(index,value){
				T.find(".switch-input").val(value.id);
				T.find(".shipping-title").html(value.title);
	...
	show_regions: function(data,search){
		...
		$.each(data,function(index,value){
			...
			if(value.continent !== "continent") T.find(".region-flag").removeClass("d-none");
	...
	get_roles: function(){
		$.ajax({
			type:"GET",
			url:"/user/ajax/catalog/shipping/get-roles",
			success: function(data){
				Shipping.show_roles(data.data);
			}
		});
	},
	show_roles: function(data){
		let select = $("#condition-role-select");
		$.each(data,function(index,value){
	...
	show_condition_type_section: function(){
		...
		$("#condition-type-"+selectedVal+"-section").removeClass("d-none");
	},
